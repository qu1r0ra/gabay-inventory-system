...
function Login({ onLoginSuccess }: { onLoginSuccess: Function }) {
    ...
    const handleLogin: React.FormEventHandler<HTMLFormElement> = (event) => {
      ...
      if (username.trim() !== '' && password.trim() !== '') {
        console.log('Login successful (debug mode)! Session set to true.');
        onLoginSuccess();
      } else {
    ...
    return (
        <div className="flex w-screen min-h-screen overflow-hidden">
            <div className="w-1/2 bg-white flex flex-col items-center justify-start"> 
                ...
                <p className='font-Work-Sans mt-2'>Don’t have an account? <span className='text-primary underline'>Sign up.</span></p>
            </div>
