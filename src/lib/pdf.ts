 ...
 * @ Modified time: 2025-07-12 13:14:07
 * @ Description:
 * 
...
import { PDFDocument, PDFPage, PDFFont, StandardFonts, setCharacterSpacing } from "pdf-lib";
...
class Page {
	...
	constructor(page: PDFPage, doc: Document, bodyFont: PDFFont, titleFont: PDFFont) {
	 ...
	 * @param size 
	 */
	header(text: string, { size = 1, alignment = "left" }: {
		size?: number,
		...
		const textWidth = this.titleFont!.widthOfTextAtSize(text, sizes[size]) + 
...
export class Document {
	 ...
	 * @param doc 
	 */
	private constructor(doc: PDFDocument, bodyFont: PDFFont, titleFont: PDFFont, filename: string) {
		this.doc = doc;
